        ...
        <main className="relative z-10 flex flex-col-reverse p-6 md:px-24 lg:pt-12 lg:min-h-[80vh] lg:grid lg:grid-cols-2">
            <div className='flex flex-col-reverse relative z-10 lg:flex-col lg:justify-between' >
                ...
                <div className="text-secondary text-center lg:text-left">
                    <h4 className='uppercase mb-4 text-[16px] lg:text-[32px] ' > {member.role} </h4>
                ...
                <div className="flex gap-x-6 my-8 justify-center lg:justify-start">
                    {
                        members.map(({name}, index) => (
                            <div onClick={()=>setActual(index)}
                                key={index} className={`w-3 aspect-square rounded-full transition cursor-pointer opacity-40 bg-white [&.active]:opacity-100 [&:not(.active)]:hover:opacity-75 ${member.name === name && 'active'}`} />
                        ))
                    }
                </div>
            </div>
            <AnimatePresence>
                <motion.div key={member.name} variants={variant} className='border-b border-white/50 w-full md:fixed md:bottom-0 md:right-0 md:w-screen md:h-[532px]  lg:h-[712px] ' >
                    <Image src={member.image} alt={member.role} className='block mx-auto w-auto h-[222px] md:h-full lg:mx-0 lg:ml-auto lg:mr-20' />
                </motion.div>
            </AnimatePresence>
