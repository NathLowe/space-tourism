            ...
            <div className="flex flex-col lg:flex-row-reverse">
                <div className="w-full lg:w-[35%] lg:min-h-[35vh] ">
                    <motion.picture key={technology.name} variants={FADE} className='block' >
                        ...
                        <Image src={technology.landscape} className='object-cover w-full h-auto' alt="Background Image" />
                    </motion.picture>
                </div>
                    ...
                    <div className="w-fit flex gap-4 mb-6 md:mb-11 lg:flex-col lg:mb-0 lg:gap-8">
                        {
                            technologies.map(({name}, index) => (
                                <div onClick={()=> setActual(index)}
                                    key={name} className={`w-10 md:w-14 aspect-square flex items-center justify-center text-white border rounded-full border-white/20 transition cursor-pointer
                                    [&.active]:border-transparent [&.active]:bg-white [&.active]:text-primary [&:not(.active)]:hover:border-white ${technology.name === name && 'active'}`}>{index+1}</div>
                            ))
                        }
                    </div>
